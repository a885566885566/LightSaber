        ...
        if(len(strs) == 2):         # Commands with two parts
            if(strs[0] == 'G00'):   # Traval
                ...
                if(preZ == upperZ): # Set traval speed
                    output_file.write('V' +str(travalSpeed)+'\n')
            elif(strs[0] == 'G01'): # Cut
                z_height = lowerZ
                if(preZ == upperZ): # Set cut speed
                    output_file.write('V' +str(cuttingSpeed)+'\n')
            preZ = z_height
            if(strs[1][0]=='Z'):
                value = lastCmd
            elif(strs[1][0]=='X'):
...
def main():
    ...
    for i in range(len(boarder_filename)):
        ...
        gcodeInterpretor(boarder_file, output_file, traval_speed[i], boarder_cutting_speed, 
