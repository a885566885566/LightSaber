...
def writeToFile(z, xy, w_fix, h_fix):
    detail = xy.split('Y')
    ...
    out = "Z%.0f,%.0f,%.0f;" %(y + w_fix, x + h_fix, z)
    # print(out)
    return out

def gcodeInterpretor(inputFile, outputFile, travalSpeed, cuttingSpeed, upperZ, lowerZ, w_fix, h_fix):
    lastCmd = ''
    preZ = upperZ
    for lines in inputFile:
        ...
        if(len(strs) == 2):         # Commands with two parts
            ...
            if(strs[1][0]=='Z'):
                ...
                # print(strs[1][0])
            if(value != ''):
                out = writeToFile(z_height, value, w_fix, h_fix)
